...
//DI GANTI MENYESUAIKAN TABLE/ KALO INI TABLE REKAM MEDIS/DIGANTI MENYESUIKAN DATA TABLE
	if (isset($_POST['btn_simpan']))
	{
		...
		{
			$sql = "INSERT INTO tb_rekammedis (id_rm, id_pasien, keluhan, id_dokter, diagnosa, id_poliklinik, riwayat, obat1, obat2, obat3, obat4, tgl_periksa ) 
			        VALUES(".$id_rm.",'".$id_pasien."','".$keluhan."', '".$id_dokter."','".$diagnosa."','".$id_poliklinik."','".$riwayat."','".$obat1."','".$obat2."','".$obat3."','".$obat4."','".$tgl_periksa."')";
...
<!--				<td><?php echo $data['id_rm']; ?>		</td>-->
				...
				<td>
				<?php  $id=$data['obat1']; 
					$sqlOBAT = "SELECT tb_obat.nama_obat FROM tb_obat WHERE id_obat=$id";
					$queryOBAT = mysqli_query($koneksi, $sqlOBAT); ?>
	             	<?php while($dataOBAT = mysqli_fetch_assoc($queryOBAT)){ ?>
					<?php echo $dataOBAT['nama_obat'];
					} ?>
								</td>
				<td><?php  $id=$data['obat2']; 
					$sqlOBAT = "SELECT tb_obat.nama_obat FROM tb_obat WHERE id_obat=$id";
					$queryOBAT = mysqli_query($koneksi, $sqlOBAT); ?>
	             	<?php while($dataOBAT = mysqli_fetch_assoc($queryOBAT)){ ?>
					<?php echo $dataOBAT['nama_obat'];
					} ?></td>
				<td><?php  $id=$data['obat3']; 
					$sqlOBAT = "SELECT tb_obat.nama_obat FROM tb_obat WHERE id_obat=$id";
					$queryOBAT = mysqli_query($koneksi, $sqlOBAT); ?>
	             	<?php while($dataOBAT = mysqli_fetch_assoc($queryOBAT)){ ?>
					<?php echo $dataOBAT['nama_obat'];
					} ?></td>
				<td><?php  $id=$data['obat4']; 
					$sqlOBAT = "SELECT tb_obat.nama_obat FROM tb_obat WHERE id_obat=$id";
					$queryOBAT = mysqli_query($koneksi, $sqlOBAT); ?>
	             	<?php while($dataOBAT = mysqli_fetch_assoc($queryOBAT)){ ?>
					<?php echo $dataOBAT['nama_obat'];
